...
# Load data
$jsonPath = Join-Path $PSScriptRoot "rabanut-scraper\jerusalem_synagogues.json"
$payload = Get-Content $jsonPath -Raw | ConvertFrom-Json

# Send
$headers = @{ "Content-Type" = "application/json"; "Authorization" = "Bearer $token" }
$total = $payload.Count
Write-Host "Sending $total synagogues to destination 331 (ירושלים)..." -ForegroundColor Cyan
