...
export class AddDestinationHierarchyAndThailand1776180000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const children = [
      ...
      { city: 'Koh Phangan', lat: 9.7439, lng: 100.0230 },
      { city: 'Phuket', lat: 7.8804, lng: 98.3923 },
      { city: 'Bangkok', lat: 13.7563, lng: 100.5018 },
      { city: 'Pai', lat: 19.3580, lng: 98.4367 },
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`DELETE FROM "destinations" WHERE "country" = $1 AND "parent_id" IS NOT NULL`, ['Thailand']);
    await queryRunner.query(`DELETE FROM "destinations" WHERE "country" = $1 AND "parent_id" IS NULL`, ['Thailand']);
    await queryRunner.query(`
      DO $$
      BEGIN
      ...
      $$;
    `);
    await queryRunner.query(`ALTER TABLE "destinations" DROP COLUMN IF EXISTS "parent_id"`);
  }
}
