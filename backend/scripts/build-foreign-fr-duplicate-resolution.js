...
/*
 * Build a review/action plan for France rows skipped due duplicate place_id.
 *
...
const inputFile = path.join(__dirname, '..', 'audit-output', 'foreign-fr-user-decisions.csv');
const outCsv = path.join(__dirname, '..', 'audit-output', 'foreign-fr-duplicate-resolution.csv');
const outHtml = path.join(__dirname, '..', 'audit-output', 'foreign-fr-duplicate-resolution.html');
...
function score(row) {
  ...
  if (Number(row.id) < 8000) n += 0.25; // prefer older internal rows only as tie-break.
  return n;
}
...
for (const group of groups) {
  const sorted = [...group].sort((a, b) => score(b) - score(a) || Number(a.id) - Number(b.id));
  const pid = sorted[0].place_id;
  if (pid === 'ChIJpeyFNvpv5kcR2A72OtJZreU') {
    // Nina Sushi: Longchamps and Voltaire are different branches; only Longchamps should get this place_id.
    sorted.forEach((row) => {
      planned.push({
        ...row,
        duplicate_action: Number(row.id) === 8262 ? 'apply_winner' : 'needs_research_not_duplicate_delete',
        duplicate_reason: Number(row.id) === 8262
          ? 'Nina Sushi Longchamps matches the Google pin.'
          : 'Nina Sushi Voltaire is a different branch; Google returned Longchamps, so do not apply this place_id and do not delete as duplicate.',
...
const rowHtml = planned.map((row) => {
  const cls = row.duplicate_action === 'apply_winner' ? 'ok' : row.duplicate_action === 'duplicate_delete_candidate' ? 'del' : 'hold';
  return `<tr class="${cls}">
...
<title>פתרון כפילויות צרפת</title>
<style>
...
th{background:#eceff1;position:sticky;top:0}.ok{background:#eef8ee}.del{background:#fff1f1}.hold{background:#fff8e1}
small{color:#607d8b}a{color:#1565c0;text-decoration:none}
</style></head><body>
<h1>פתרון כפילויות Google place_id - צרפת</h1>
<div class="sum">
  ...
  <span>לא למחוק, חיפוש מחדש: ${counts.needs_research_not_duplicate_delete || 0}</span>
</div>
<p>זה דוח החלטה בלבד. אין כאן כתיבה ל-DB.</p>
<table><thead><tr><th>פעולה</th><th>ID</th><th>שלנו</th><th>גוגל</th><th>נימוק</th><th>קישור</th></tr></thead><tbody>
${rowHtml}
