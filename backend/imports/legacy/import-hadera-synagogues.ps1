...
# Load data
$jsonPath = Join-Path $PSScriptRoot "rabanut-scraper\hadera_synagogues.json"
$payload = Get-Content $jsonPath -Raw | ConvertFrom-Json

# Send
$headers = @{ "Content-Type" = "application/json"; "Authorization" = "Bearer $token" }
$total = $payload.Count
Write-Host "Sending $total synagogues to destination 311 (חדרה)..." -ForegroundColor Cyan
