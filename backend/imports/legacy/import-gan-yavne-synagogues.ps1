...
# Load data
$jsonPath = Join-Path $PSScriptRoot "rabanut-scraper\gan-yavne_synagogues_import.json"
$payload = Get-Content $jsonPath -Raw | ConvertFrom-Json

# Send
$headers = @{ "Content-Type" = "application/json"; "Authorization" = "Bearer $token" }
$total = $payload.Count
Write-Host "Sending $total synagogues to destination 329 (גן יבנה)..." -ForegroundColor Cyan
