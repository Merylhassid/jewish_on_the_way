...
const DESTINATION_ID = 359;
const CITY           = 'הרצליה';
const GMAPS_URL      = 'https://www.google.com/maps/search/%D7%91%D7%99%D7%AA+%D7%9B%D7%A0%D7%A1%D7%AA+%D7%94%D7%A8%D7%A6%D7%9C%D7%99%D7%94%E2%80%AD/@32.0832842,34.8547716,14z?hl=iw';
const OUTPUT_FILE    = path.join(__dirname, 'herzliya_synagogues.json');
const PROGRESS_FILE  = path.join(__dirname, 'herzliya_progress.json');
...
(async () => {
  ...
  for (let i = 0; i < placeLinks.length; i++) {
    ...
    try {
      ...
      if (address && !address.includes(CITY) && !address.includes('הרצליה')) {
  ...
  log(`\n✅ Done! ${results.length} synagogues saved to herzliya_synagogues.json`);
