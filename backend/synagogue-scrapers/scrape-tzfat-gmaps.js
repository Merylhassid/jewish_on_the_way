...
const DESTINATION_ID = 529;
const CITY           = 'צפת';
const GMAPS_URL      = 'https://www.google.com/maps/search/%D7%91%D7%99%D7%AA+%D7%9B%D7%A0%D7%A1%D7%AA+%D7%A6%D7%A4%D7%AA/@32.9641,35.4956,14z?hl=iw';
const OUTPUT_FILE    = path.join(__dirname, 'tzfat_synagogues.json');
const PROGRESS_FILE  = path.join(__dirname, 'tzfat_progress.json');
...
(async () => {
  ...
  if (hasFeed) {
    ...
    if (h1Text) {
      ...
      log('⚠️ No feed and no place page found');
    }
  }
  ...
  for (let i = 0; i < placeLinks.length; i++) {
    ...
    try {
      ...
      if (address && !address.includes(CITY) && !address.includes('צפת')) {
  ...
  log(`\n✅ Done! ${results.length} synagogues saved to tzfat_synagogues.json`);
