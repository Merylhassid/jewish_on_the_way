...
const DESTINATION_ID = 506;
const CITY           = 'נשר';
const GMAPS_URL      = 'https://www.google.com/maps/search/%D7%91%D7%99%D7%AA+%D7%9B%D7%A0%D7%A1%D7%AA+%D7%91%D7%A0%D7%A9%D7%A8%E2%80%AD/@32.7684899,35.0534934,14z?hl=iw';
const OUTPUT_FILE    = path.join(__dirname, 'nesher_synagogues.json');
const PROGRESS_FILE  = path.join(__dirname, 'nesher_progress.json');
...
(async () => {
  ...
  for (let i = 0; i < placeLinks.length; i++) {
    ...
    try {
      ...
      if (address && !address.includes(CITY) && !address.includes('נשר')) {
  ...
  log(`\n✅ Done! ${results.length} synagogues saved to nesher_synagogues.json`);
