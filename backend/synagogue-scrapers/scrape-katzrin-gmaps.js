...
const DESTINATION_ID = 533;
const CITY           = 'קצרין';
const GMAPS_URL      = 'https://www.google.com/maps/search/%D7%91%D7%99%D7%AA+%D7%9B%D7%A0%D7%A1%D7%AA+%D7%A7%D7%A6%D7%A8%D7%99%D7%9F/@32.9908739,35.6899835,14z?hl=iw';
const OUTPUT_FILE    = path.join(__dirname, 'katzrin_synagogues.json');
const PROGRESS_FILE  = path.join(__dirname, 'katzrin_progress.json');
...
(async () => {
  ...
  // Check if we landed on a single place page (small city) or a results list
  ...
  if (hasFeed) {
    ...
    // Single place page — treat current page as the only result
    log('📄 Single place page detected (small city)');
    placeLinks = [{ href: page.url(), name: '' }];
  }
  ...
  for (let i = 0; i < placeLinks.length; i++) {
    ...
    try {
      ...
      if (address && !address.includes(CITY) && !address.includes('קצרין')) {
  ...
  log(`\n✅ Done! ${results.length} synagogues saved to katzrin_synagogues.json`);
