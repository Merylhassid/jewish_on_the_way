...
const DESTINATION_ID = 494;
const CITY           = 'אופקים';
const GMAPS_URL      = 'https://www.google.com/maps/search/%D7%91%D7%99%D7%AA+%D7%9B%D7%A0%D7%A1%D7%AA+%D7%90%D7%95%D7%A4%D7%A7%D7%99%D7%9D/@31.309483,34.6287222,14z?hl=iw';
const OUTPUT_FILE    = path.join(__dirname, 'ofakim_synagogues.json');
const PROGRESS_FILE  = path.join(__dirname, 'ofakim_progress.json');
...
(async () => {
  ...
  for (let i = 0; i < placeLinks.length; i++) {
    ...
    try {
      ...
      if (address && !address.includes(CITY) && !address.includes('אופקים')) {
  ...
  log(`\n✅ Done! ${results.length} synagogues saved to ofakim_synagogues.json`);
